...
$(SYNTH_TARGETS): %.synth: ${DIGITAL}/design/%.v
	SYNTH_VERILOG=${DIGITAL}/design/$*.v SYNTH_TARGET=${SYNTH}/$* yosys -c ${SCRIPT}/synth.tcl
...
$(SIM_SO_OBJ): ${BUILD}/%.so:
	ngspice vlnggen -- --CFLAGS -DVL_TIME_STAMP64 --CFLAGS -DVL_NO_LEGACY ${DIGITAL}/design/$*.v
	mv $*.so ${BUILD}/$*.so
	rm -rf $*_obj_dir
