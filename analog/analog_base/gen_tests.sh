...
if [ "$1" = "1px" ]; then
    ...
    corners=( tt ss fs ) # ss ff sf fs 
    flavors=( xx ) # xx px xm pm 
    seeds=(0 64 128 192)
    scnt=64
    parg="pixels=eval:1"
elif [ "$1" = "1qx" ]; then
...
for seed in ${seeds[@]}; do
    [ -f seeds_$seed ]    || python ../../script/gen_seeds.py -s 0 -p 5 -i 1 -r 1           -n $scnt         --start $seed > seeds_$seed
    [ -f seeds_rr_$seed ] || python ../../script/gen_seeds.py -s 0 -p 5 -i 0 -r 1.8 -t 2 -d -n $((scnt * 4)) --start $seed --prefix randvc > seeds_rr_$seed
done
...
for flavor in ${flavors[@]}; do
    ...
    for corner in ${corners[@]}; do
        for seed in ${seeds[@]}; do
            ...
            python ../../script/teng.py -o runme_${pixels}_${corner}_${flavor}_${seed}.cir adc_sky_5px.temp.cir "${args[@]}" $flv 
            echo
        done
    done
done
